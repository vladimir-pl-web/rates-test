...
const Converter: FC<IConverter> = ({ name, locale, rate }) => {
  ...
  return (
    <div className={cn(styles.converter)}>
      <TypographyP>{infoString}</TypographyP>
      <div className={cn(styles.content)}>
        <div className={styles.inputs}>
          <div className={cn(styles.inputBox)}>
            ...
            />
            <div className={cn(styles.label)}>{ascending ? name : locale}</div>
          </div>
          ...
          <div className={cn(styles.inputBox)}>
            <Input
              ...
              disabled={true}
            />
            <div className={cn(styles.label)}>{ascending ? locale : name}</div>
          </div>
        </div>
        <div className={cn(styles.button)}>
          <CustomButton onClick={onDirectionHandler}>
            <Image src={arrows} alt="arrows" width={30} height={30} />
          </CustomButton>
        </div>
      </div>
    </div>
